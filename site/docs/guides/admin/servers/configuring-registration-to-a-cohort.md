...
An [OMAG Server](./concepts/omag-server) that is capable of being a
[Cohort Member](./concepts/cohort-member) can register with one or more
...
These are topics on an [event bus](./concepts/event-bus) that the members use to exchange information.
...
The result of the cohort configuration call fills out an entry in the cohort list of the server's [configuration document](./concepts/configuration-document).
